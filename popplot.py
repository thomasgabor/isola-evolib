...
def popplot(self, **kwargs):
    args = cp(self.global_args)
    args.update(kwargs)
    evolution = 0
    for name,measure in measurements.items():
        for gen in range(0, len(self.all_populations[evolution][0])):
            fig = plt.figure()
            ...
            if gen == 0 and not args.get('no_show', False):
                plt.show()
                pass
            plt.savefig(self.experiment_filename() + '.test=' + str(name) + '.gen' + str(gen) + '.png')
    if not args.get('no_show', False):
        plt.show()



explib.Experiment.popplot = popplot
explib.Experiment.popplot(exp, plot_base_fitness=False, no_show=True, zoom=1, approx_surface=False, approx_resolution=10)